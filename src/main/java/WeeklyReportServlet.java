...
@MultipartConfig // 👈 CRITICAL: Tells Tomcat/Render how to process Excel file binary uploads
public class WeeklyReportServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // ==========================================
        // 🚀 NEW LOGIC: HANDLES THE BREVO EMAIL PIPELINE
        // ==========================================
        if ("exportEmail".equals(action)) {
            response.setContentType("application/json");
            PrintWriter out = response.getWriter();
            try {
                // 1. Grab the uploaded binary excel part
                Part filePart = request.getPart("file"); 
                if (filePart == null) {
                    out.print("{\"success\":false,\"message\":\"No file payload found\"}");
                    return;
                }

                // 2. Stream byte buffers cleanly 
                ...
                // 3. Dispatch directly to our Brevo API pipeline handler
                ExcelEmailSender.sendReportWithAttachment(excelBytes, "Weekly_Status_Report.xlsx", userEmail);
        ...
        // Original database table mappings continue...
