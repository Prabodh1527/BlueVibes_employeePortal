...
public class UpdateProfileServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // 2. Refresh Qualifications
            try (PreparedStatement delQual = con.prepareStatement("DELETE FROM user_qualifications WHERE user_email=?")) {
            ...
            if (qualNames != null) {
                ...
                try (PreparedStatement psQual = con.prepareStatement(insQual)) {
            ...
            // 3. Refresh Experience
            try (PreparedStatement delExp = con.prepareStatement("DELETE FROM user_experience WHERE user_email=?")) {
            ...
            if (expComps != null) {
                String insExp = "INSERT INTO user_experience (user_email, company_name, designation, joined_date, left_date) " +
                                "VALUES (?,?,?,CAST(? AS DATE),CAST(? AS DATE))";
                try (PreparedStatement psExp = con.prepareStatement(insExp)) {
                    for (int i = 0; i < expComps.length; i++) {
                        if (expComps[i] != null && !expComps[i].trim().isEmpty()) {
                            ...
                            psExp.setString(4, (expJoins[i] == null || expJoins[i].isEmpty()) ? null : expJoins[i]);
                            psExp.setString(5, (expLefts[i] == null || expLefts[i].isEmpty()) ? null : expLefts[i]);
                            psExp.executeUpdate();
                        }
                    }
                }
            ...
            try (PreparedStatement psPaths = con.prepareStatement("SELECT certification_name, certificate_path FROM user_certifications WHERE user_email=?")) {
                psPaths.setString(1, email);
                try (ResultSet rsPaths = psPaths.executeQuery()) {
                    while(rsPaths.next()) existingPaths.put(rsPaths.getString("certification_name"), rsPaths.getString("certificate_path"));
                }
            }

            try (PreparedStatement delCert = con.prepareStatement("DELETE FROM user_certifications WHERE user_email=?")) {
            ...
            if (certNames != null) {
                ...
                String insCert = "INSERT INTO user_certifications (user_email, certification_name, issue_date, certificate_path) " +
                                 "VALUES (?,?,CAST(? AS DATE),?)";
                try (PreparedStatement psCert = con.prepareStatement(insCert)) {
                    for (int i = 0; i < certNames.length; i++) {
                        if (certNames[i] != null && !certNames[i].trim().isEmpty()) {
                            ...
                            psCert.setString(3, (certDates[i] == null || certDates[i].isEmpty()) ? null : certDates[i]);
